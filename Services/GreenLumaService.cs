...
public partial class GreenLumaService
{
    ...
    /// <summary>
    /// Runs pre-launch diagnostics and returns a list of issues found.
    ...
    public static List<string> RunPreLaunchDiagnostics(Config config)
    {
        ...
        else
        {
            ...
            if (dllPath != null)
            {
                if (!File.Exists(dllPath))
                    issues.Add($"GreenLuma DLL not found: {Path.GetFileName(dllPath)} — likely quarantined by antivirus.");
                else
                {
                    // Check if DLL is suspiciously small (truncated/corrupted)
        ...
        // Check for conflicting overlay DLLs in Steam path
    ...
    /// <summary>
    /// Monitors Steam after injection and returns crash diagnostic info.
    /// Call this after LaunchGreenLumaAsync returns true.
    /// </summary>
    public static async Task<string?> MonitorSteamAfterLaunchAsync(Config config, int timeoutSeconds = 30)
    {
        return await Task.Run(() =>
        {
            try
            {
                ...
                // Wait for Steam process to appear
                ...
                {
                    // Check if injector itself crashed
                ...
                // Monitor Steam for early exit using polling (can't use WaitForExit on externally started processes)
                ...
                while (elapsed < remainingMs)
                {
                    ...
                    try
                    {
                        ...
                        {
                            int exitCode;
                            try { exitCode = steamProcess.ExitCode; }
                            catch { exitCode = -1; }
                            steamProcess.Dispose();

                            // Steam exited within our monitoring window — likely a crash
                ...
                // Steam is still running after timeout — good sign
                steamProcess.Dispose();
        ...
        try
        {
            ...
            while ((record = reader.ReadEvent()) != null)
            {
                using (record)
                {
                    ...
                    {
                        // Extract faulting module from the event
    ...
    private static string? CheckRecentDefenderDetections(string greenLumaPathLower)
    {
        try
        {
            ...
            while ((record = reader.ReadEvent()) != null)
            {
                using (record)
                {
                    ...
                    {
                        // Extract the detected file name
                        var lines = desc.Split('\n');
                        foreach (var line in lines)
        ...
        catch { /* Event log may not be accessible */ }
