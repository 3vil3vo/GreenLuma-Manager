...
public partial class UpdateService
{
    ...
    public static async Task<bool> PerformAutoUpdateAsync(string downloadUrl)
    {
        try
        {
            ...
            {
                LogService.LogError("UpdateService.PerformAutoUpdate", new InvalidOperationException($"Blocked untrusted download URL: {downloadUrl}"));
                return false;
            }
            ...
            if (!IsValidPeFile(tempExePath))
            {
                ...
                LogService.LogError("UpdateService.PerformAutoUpdate", new InvalidOperationException("Downloaded file is not a valid PE executable"));
                return false;
            }
